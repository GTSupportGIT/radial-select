const sharedStyles = `
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .trigger:hover { transform: scale(1.03); }
  ...
  .option {
    ...
    border: 1px solid var(--radial-option-border);
    border-radius: var(--radial-option-radius);
    clip-path: var(--radial-shape-clip);
    background: var(--radial-option-bg);
    color: var(--radial-option-text);
    ...
    transform:
      ...
      rotate(var(--radial-option-rotate))
      scale(var(--slot-scale, 1));
    transition:
      transform var(--radial-transition),
      opacity var(--radial-transition),
      background var(--radial-transition),
      color var(--radial-transition),
      border-color var(--radial-transition);
    z-index: 2;
  }

  .option[aria-selected="true"] {
    background: color-mix(in srgb, var(--radial-primary), white 84%);
    border-color: var(--radial-primary);
  }
  ...
  :host([shape="petal"]) {
    --radial-option-radius: 68% 68% 44% 44%;
    --radial-option-width: 96px;
    --radial-option-height: 118px;
  }

  :host([shape="hex"]) {
    --radial-option-radius: 0px;
  ...
  @media (prefers-reduced-motion: reduce) {
    :host { --radial-transition: 1ms linear; }
  }
`;
...
class SelectorBase extends HTMLElement {
  ...
  static get observedAttributes() {
    return ['value', 'label', 'hint', 'disabled', 'open'];
  }
  ...
  get value() {
    return this.getAttribute('value') ?? this.querySelector(':scope > option[selected]')?.value ?? '';
  }
  ...
  _choose(option) {
    ...
    [...this.querySelectorAll(':scope > option')].forEach((el) => {
      el.selected = el.value === option.value;
    });
...
export class RadialSelect extends SelectorBase {
  ...
  _renderOptions(options) {
    ...
    if (!count) return;
    const radius = parseFloat(getComputedStyle(this).getPropertyValue('--radial-radius')) || 128;
    options.slice(0, 8).forEach((option, index) => {
      ...
      button.style.setProperty('--slot-opacity', '1');
      button.style.setProperty('--slot-scale', '1');
...
export class RotarySelect extends SelectorBase {
  static get observedAttributes() {
    return [...super.observedAttributes, 'visible-count'];
  }
  ...
  _renderOptions(options) {
    ...
    for (let i = 0; i < visibleCount; i++) {
      ...
      button.style.setProperty('--slot-x', `${Math.cos(radians) * radius}px`);
      button.style.setProperty('--slot-y', `${Math.sin(radians) * radius}px`);
      button.style.setProperty('--slot-opacity', String(Math.max(.14, 1 - edge * .72)));
      button.style.setProperty('--slot-scale', String(1 - edge * .18));
      button.addEventListener('click', (event) => {
        event.stopPropagation();
        this._choose(option);
      });
