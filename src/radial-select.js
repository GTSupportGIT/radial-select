const sharedStyles = `
  :host {
    ...
    --radial-option-height: 112px;
    --radial-option-radius: 48% 48% 42% 42% / 58% 58% 44% 44%;
    ...
    --radial-shadow: 0 18px 42px rgba(23,75,58,.15);
    --radial-option-shadow: 0 11px 26px rgba(23,75,58,.10);
    ...
    /* Flower palette: deliberately exposed for easy theming. */
    --radial-petal-1: #ffd8e8;
    --radial-petal-2: #ffe5b7;
    --radial-petal-3: #fff3a8;
    --radial-petal-4: #d9f3bd;
    --radial-petal-5: #bfeee4;
    --radial-petal-6: #c8e2ff;
    --radial-petal-7: #ddd0ff;
    --radial-petal-8: #f1cff3;
  ...
  *, *::before, *::after { box-sizing: border-box; }
  ...
  .trigger-label {
    max-width: calc(var(--radial-trigger-size) - 12px);
    overflow: hidden;
  ...
  .option {
    ...
    border: 1px solid var(--slot-border, var(--radial-option-border));
    border-radius: var(--radial-option-radius);
    clip-path: var(--radial-shape-clip);
    background: var(--slot-bg, var(--radial-option-bg));
    color: var(--slot-text, var(--radial-option-text));
    box-shadow: var(--slot-shadow, var(--radial-option-shadow));
    font: inherit;
    ...
    transition: transform var(--radial-transition), opacity var(--radial-transition),
      ...
      border-color var(--radial-transition), box-shadow var(--radial-transition);
    z-index: 2;
  }
  ...
  :host([shape="petal"]) {
    ...
    --radial-option-radius: 54% 54% 44% 44% / 64% 64% 36% 36%;
    --radial-shape-clip: none;
  }
  :host([shape="hex"]) {
  ...
  :host([shape="tech"]) {
    --radial-option-width: 104px;
    --radial-option-height: 80px;
    --radial-option-radius: 18px 36px 18px 36px;
    --radial-shape-clip: none;
  }

  /* Built-in flower treatment: soft coloured petals like the original concept. */
  :host([theme="flower"]) {
    --radial-primary: #f4be3d;
    --radial-primary-text: #563e00;
    --radial-option-text: #4f4650;
    --radial-option-border: rgba(111,96,112,.18);
    --radial-shadow: 0 18px 42px rgba(116,83,84,.17);
    --radial-option-shadow: 0 10px 24px rgba(97,76,92,.13);
    --radial-trigger-bg: #fffaf0;
    --radial-trigger-text: #9c6b16;
    --radial-trigger-border: #f2d596;
    --radial-arrow-bg: #fffaf3;
    --radial-arrow-text: #8d661a;
  }

  /* Built-in future treatment: cool dark hub, cyan edges and luminous choices. */
  :host([theme="future"]) {
    --radial-primary: #071f2b;
    --radial-primary-text: #d9fbff;
    --radial-option-bg: rgba(8,35,47,.92);
    --radial-option-text: #bffaff;
    --radial-option-border: #41e6ee;
    --radial-shadow: 0 0 0 1px rgba(65,230,238,.35), 0 0 32px rgba(65,230,238,.24);
    --radial-option-shadow: 0 0 0 1px rgba(65,230,238,.25), 0 0 18px rgba(65,230,238,.16);
    --radial-trigger-bg: #081e29;
    --radial-trigger-text: #7ff7ff;
    --radial-trigger-border: #41e6ee;
    --radial-arrow-bg: #0b2632;
    --radial-arrow-text: #87fbff;
  }
  :host([theme="future"]) .option[aria-selected="true"] {
    --slot-bg: rgba(16,67,80,.98) !important;
    --slot-shadow: 0 0 0 2px #7ff7ff, 0 0 28px rgba(65,230,238,.38) !important;
  }
...
function makeTemplate({ rotary = false } = {}) {
  ...
  template.innerHTML = `
    <style>
      ...
      .arrow {
        width: var(--radial-arrow-size);
        height: var(--radial-arrow-size);
        border-radius: 50%;
        border: 1px solid color-mix(in srgb,var(--radial-arrow-text),transparent 70%);
        background: var(--radial-arrow-bg);
        color: var(--radial-arrow-text);
        box-shadow: var(--radial-option-shadow);
        font: inherit;
        font-size: 2rem;
        line-height: 1;
        cursor: pointer;
...
class SelectorBase extends HTMLElement {
  ...
  _makeOption(option, angle, radius, opacity=1, scale=1) {
    ...
    if (this.getAttribute('theme') === 'flower') {
      const paletteIndex = (option.index % 8) + 1;
      button.style.setProperty('--slot-bg',`var(--radial-petal-${paletteIndex})`);
      button.style.setProperty('--slot-border','rgba(91,75,91,.22)');
      button.style.setProperty('--slot-text','#4c4148');
    }
